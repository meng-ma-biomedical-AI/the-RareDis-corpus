...
def order_brat(path):
    ...
    try:
        ...
        for file in files:
            ...
            gc.collect()
            result_entities_df.sort_values(by=char_c_number, na_position='first', inplace=True)
            ...
            result_entities_df.drop(columns=['id'], inplace=True)
            ...
            if len(relations_df.index) != 0:
                split_relations_df = relations_df['type'].str.replace('T', '').str.replace(':', ' ').str.split(' ',
                                                                                                               expand=True)
        ...
        print('Process order BRAT files is completed in {:.2f} seconds'.format(time.time() - start_time))
...
def count_entities(path, count_df=None):
    ...
    try:
        ...
        del total_df
        gc.collect()
        print('Entities count of BRAT files is completed in {:.2f} seconds'.format(time.time() - start_time))
...
def main():
    ...
    if len(sys.argv) == 1:
        try:
            dir_path = r"C:\Users\Usuario\Desktop\UC3M_NLP\annotations\gold_nlp4rare_corpus\dev"  # Replace by desired directory path
